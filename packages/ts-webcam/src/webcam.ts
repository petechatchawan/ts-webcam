...
import { DEFAULT_WEBCAM_CAPABILITIES, DEFAULT_WEBCAM_CONFIG, DEFAULT_WEBCAM_STATE } from "./constants";
...
import { buildMediaConstraints, createResolution, stopMediaStream, validatePermissions } from "./utils";
...
export class Webcam {
	...
	public async start(): Promise<void> {
		...
		try {
			...
			if (error instanceof WebcamError) {
				...
				this.handleError(new WebcamError("STREAM_ERROR", "Failed to start webcam", error as Error));
			}
	...
	public async setZoomLevel(zoomLevel: number): Promise<void> {
		...
		try {
			...
			throw new WebcamError("STREAM_ERROR", "Failed to set zoom level", error as Error);
		}
	}
	...
	public async enableTorch(active: boolean): Promise<void> {
		...
		try {
			...
			throw new WebcamError("STREAM_ERROR", "Failed to set torch mode", error as Error);
		}
	}
	...
	public async setFocusMode(mode: string): Promise<void> {
		...
		try {
			...
			throw new WebcamError("STREAM_ERROR", "Failed to set focus mode", error as Error);
		}
	}
	...
	public hasPermissionDenied(): boolean {
		return (
			...
			(!!this.state.configuration?.enableAudio && this.state.permissionStates.microphone === "denied")
		);
	}
	 ...
	 */
	public async getDeviceCapabilities(deviceId: string): Promise<DeviceCapabilities> {
		try {
			...
			throw new WebcamError("STREAM_ERROR", "Failed to check device capabilities", error as Error);
		}
	}
	...
	private async getAvailableDevices(): Promise<MediaDeviceInfo[]> {
		try {
			...
			this.handleError(new WebcamError("DEVICE_NOT_FOUND", "Failed to get device list", error as Error));
			return [];
		}
	}
	...
	public async getAudioOutputDevices(): Promise<MediaDeviceInfo[]> {
		...
		return devices.filter((device: MediaDeviceInfo) => device.kind === "audiooutput");
	}
	...
	private async openWebcam(): Promise<void> {
		if (!this.state.configuration!.preferredResolutions) {
			...
			try {
				...
				throw new WebcamError("STREAM_ERROR", "Failed to open webcam with supported resolution", error as Error);
			}
		}
		...
		if (this.state.configuration!.allowFallbackResolution) {
			try {
				...
				throw new WebcamError("STREAM_ERROR", "Failed to open webcam with any resolution", lastError || undefined);
			}
		} else {
			throw lastError;
		}
	...
	private async tryAnyResolution(): Promise<void> {
		...
		try {
			...
			throw new WebcamError("STREAM_ERROR", "Failed to initialize webcam with any resolution", error as Error);
		}
	}
