...
export class TsWebcam {
    ...
    async checkPermissions(): Promise<Record<string, PermissionState>> {
        ...
        try {
            // @ts-ignore: navigator.permissions might not be available in some browsers
    ...
    async getDeviceCapabilities(deviceId: string): Promise<DeviceCapability> {
        ...
        try {
            ...
            return {
                ...
                supportedFrameRates: capabilities.frameRate && capabilities.frameRate.min !== undefined && capabilities.frameRate.max !== undefined
    ...
    private _handleStartCameraError(error: any): WebcamError {
        ...
        if (error?.name === 'NotAllowedError') {
            return new WebcamError('Camera access denied', WebcamErrorCode.PERMISSION_DENIED);
        }

        if (error?.name === 'NotFoundError') {
            return new WebcamError('Camera device not found', WebcamErrorCode.DEVICE_NOT_FOUND);
        }

        if (error?.name === 'NotReadableError') {
            return new WebcamError('Camera is already in use', WebcamErrorCode.DEVICE_BUSY);
        }

        if (error?.name === 'OverconstrainedError') {
            return new WebcamError(`Camera constraints not satisfied`, WebcamErrorCode.OVERCONSTRAINED);
        }
        ...
        return new WebcamError(
            `Failed to start camera: ${details || 'Unknown error'}`,
