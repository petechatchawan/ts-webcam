...
export class Webcam {
    ...
    public isActive(): boolean {
        return (
            this.state.activeStream !== null && this.state.activeStream.active
        );
    }
    ...
    public async setZoom(zoomLevel: number): Promise<void> {
        if (
            ...
            !this.state.capabilities.zoomSupported
        ) {
    ...
    public async setFocusMode(mode: string): Promise<void> {
        if (
            !this.state.activeStream ||
            !this.state.capabilities.focusSupported
        ) {
            throw new WebcamError(
                'focus-not-supported',
    ...
    public needsPermissionRequest(): boolean {
        return (
            this.state.permissions.camera === 'prompt' ||
            (!!this.state.config?.audioEnabled &&
                this.state.permissions.microphone === 'prompt')
        );
    }
    ...
    private async setupPreviewElement(): Promise<void> {
        if (this.state.config!.previewElement && this.state.activeStream) {
            this.state.config!.previewElement.srcObject =
                this.state.activeStream;
            this.state.config!.previewElement.style.transform = this.state
                .config!.mirrorEnabled
                ? 'scaleX(-1)'
