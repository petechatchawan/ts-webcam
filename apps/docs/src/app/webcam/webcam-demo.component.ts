...
export class WebcamDemoComponent implements AfterViewInit {
  ...
  private async checkDeviceCapabilities(): Promise<void> {
    try {
      ...
      result.resolutions.forEach((res: any) => {
        console.log(
          `${res.name} (${res.width}x${res.height}): ${res.supported ? 'Supported' : 'Not supported'
          }`,
        );
      });
  ...
  private async showMessage(
    ...
    const panelClass = type === 'success' ? 'green-snackbar' :
      type === 'warning' ? 'orange-snackbar' : 'red-snackbar';
