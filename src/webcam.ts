...
export class Webcam {
    ...
    public async setZoom(zoomLevel: number): Promise<void> {
        ...
        try {
            ...
            await videoTrack.applyConstraints({
                advanced: [{ zoom: constrainedZoomLevel } as ExtendedMediaTrackConstraintSet],
            });
    ...
    public needsPermissionRequest(): boolean {
        return (
            this.state.permissions.camera === 'prompt' ||
            (!!this.state.config?.audioEnabled && this.state.permissions.microphone === 'prompt')
        );
    }
    ...
    public hasPermissionDenied(): boolean {
        return (
            ...
            (!!this.state.config?.audioEnabled && this.state.permissions.microphone === 'denied')
        );
    }
    ...
    private handleError(error: Error): void {
        ...
        this.state.config?.onError?.(this.state.lastError as WebcamError);
    }
